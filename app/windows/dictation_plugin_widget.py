...
class DictationPluginWidget(QWidget):
    ...
    Задачи вкладки:
    - показать состояние plugin и runtime-режима диктовки;
    - дать ручное включение/выключение режима диктовки;
    - показать команды dictation plugin из plugin resources.
    """
    ...
    def _build_ui(self):
        ...
        description = QLabel(
            ...
            "вручную включить или выключить режим диктовки и увидеть команды, "
    ...
    def _is_dictation_plugin_enabled(self) -> bool:
        ...
        if isinstance(enabled_map, dict):
            return bool(enabled_map.get("dictation", True))

        legacy_enabled = self.config.get("assistant", {}).get("enabled_plugins")
        if isinstance(legacy_enabled, list):
            return "dictation" in {str(item) for item in legacy_enabled}
